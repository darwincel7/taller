...
export const StorePurchasesTab = () => {
  ...
  const purchases = useMemo(() => {
    return inventory.filter(p => parseInventoryCategory(p.category).type === 'STORE_PURCHASE');
  }, [inventory]);

  const registerAccountingExpense = async (title: string, amount: number) => {
    try {
  ...
  const handleCreatePurchase = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.providerId || formData.amount <= 0) {
