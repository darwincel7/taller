            ...
            <div className="p-6 pl-8 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-wrap gap-1.5">
                        {isReturn ? (
                            <span className="text-[8px] sm:text-[9px] font-black bg-red-600 text-white px-1.5 sm:px-2 py-0.5 rounded-full flex items-center gap-1"><RotateCcw className="w-2.5 h-2.5 sm:w-3 sm:h-3"/> {order.returnRequest?.status === 'PENDING' ? 'SOLICITUD DEVOLUCIÓN' : 'DEVOLUCIÓN'}</span>
                        ) : (
                        ...
                        {order.externalRepair && (
                            <span className={`text-[8px] sm:text-[9px] font-black px-1.5 sm:px-2 py-0.5 rounded-full flex items-center gap-1 border ${order.externalRepair.status === 'PENDING' ? 'bg-purple-100 text-purple-700 border-purple-200 animate-pulse' : 'bg-purple-600 text-white border-purple-600'}`}><Truck className="w-2.5 h-2.5 sm:w-3 sm:h-3"/> {order.externalRepair.targetWorkshop} {order.externalRepair.status === 'PENDING' ? '(PEND.)' : ''}</span>
                        )}
                    </div>
        ...
        <div className="flex items-center gap-2 group">
            ...
            >
                ...
                <div className="text-right shrink-0 ml-2 hidden sm:block">
                    ...
                    <p className="text-sm sm:text-base font-black text-slate-700">${(order.finalPrice || order.estimatedCost).toLocaleString()}</p>
                </div>
            </div>
            <button 
...
export const OrderList: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto pb-20 font-sans bg-slate-50 min-h-screen relative">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4 sm:gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full xl:w-auto">
          ...
          <div className="flex bg-white p-1 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm shrink-0">
              <button onClick={() => setViewMode('CARDS')} className={`p-1.5 sm:p-2 rounded-lg sm:rounded-xl transition-all ${viewMode === 'CARDS' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`} title="Vista de Tarjetas"><LayoutGrid className="w-4 h-4 sm:w-5 sm:h-5" /></button>
              <button onClick={() => setViewMode('TABLE')} className={`p-1.5 sm:p-2 rounded-lg sm:rounded-xl transition-all ${viewMode === 'TABLE' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`} title="Vista de Tabla Densa"><List className="w-4 h-4 sm:w-5 sm:h-5" /></button>
          </div>
          ...
          <button onClick={() => setShowDbFixModal(true)} className="p-2 sm:p-3 bg-white border border-slate-200 rounded-xl sm:rounded-2xl text-slate-700 hover:bg-slate-50 transition shadow-sm shrink-0 hidden sm:block" title="Reparar Base de Datos"><Database className="w-4 h-4 sm:w-5 sm:h-5" /></button>
      ...
      {/* FILTER TABS */}
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="flex bg-white p-1 rounded-xl sm:rounded-2xl border border-slate-200 w-full lg:w-auto shadow-sm overflow-x-auto custom-scrollbar no-scrollbar">
            {[
              ...
              { id: 'RECIBIDOS', label: 'Recibidos', count: counts.store, icon: ShoppingBag, isStore: true }, 
              { id: 'GARANTIAS', label: 'Garantías', count: counts.warranty, icon: ShieldAlert },
              { id: 'EXTERNAL', label: 'Externo', count: counts.external, icon: Truck },
              { id: 'HISTORIAL', label: 'Entregados', count: counts.history, icon: History },
              ...
              >
                {React.createElement(tab.icon, { className: "w-3 h-3 sm:w-4 sm:h-4" })} <span className="hidden sm:inline">{tab.label}</span><span className="sm:hidden">{tab.label.split('/')[0]}</span> 
                <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] ${filterTab === tab.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500 border'}`}>{tab.count}</span>
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2 sm:gap-3 w-full lg:w-auto">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="flex-1 lg:flex-none bg-white border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-xs font-black text-slate-700 outline-none shadow-sm focus:ring-2 focus:ring-blue-100 w-full">
