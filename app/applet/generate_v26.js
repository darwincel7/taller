...
-- 2. Apply V25 Migrations (Adding missing columns if they already exist but lack them)
...
export const DbFixModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const FULL_SQL = atob('${encodedSql}');
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-800 dark:text-white">Plan V26 - Creacion de Tablas Faltantes + RPC</h2>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="bg-amber-50 border border-amber-200 text-amber-800 dark:bg-amber-900/20 dark:border-amber-800/30 dark:text-amber-300 p-4 rounded-xl mb-6 text-sm">
            <h3 className="font-bold flex items-center gap-2 mb-2">
              ...
              Importante (V26) - Tablas Ausentes
            </h3>
            <p className="mb-2">El error <b>42P01: relation no existe</b> sucedía porque las tablas base (como inventory_movements) no se habían creado todavia en esta base de datos. Esta nueva consulta creará primero TODAS las tablas necesarias si no existen (inventory_movements, pos_sales, cash_movements, client_credits) y luego aplicará las migraciones y RPCs.</p>
          </div>
...
console.log('Successfully wrote DbFixModal.tsx at V26!');
