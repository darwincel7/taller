...
SELECT 
    ...
    COALESCE((SELECT sum(unit_cost * quantity) FROM public.pos_sale_items WHERE sale_id = ps.id), 0) as cost_amount,
    ps.total - COALESCE((SELECT sum(unit_cost * quantity) FROM public.pos_sale_items WHERE sale_id = ps.id), 0) as net_profit,
...
-- Nota: Aquí contamos el INGRESO real por pagos, no el total facturado si no se ha cobrado
SELECT 
...
SELECT 
    ...
    COALESCE(o."currentBranch", 'T4') as branch,
    op.cashier_id as user_id,
    op.amount as gross_amount, -- Será negativo
    0 as cost_amount,
    op.amount as net_profit,
    'Reembolso Taller: ' || COALESCE(o."deviceModel", 'Equipo') as description,
    'completed' as status
FROM 
    public.order_payments op
JOIN 
